...
typedef union
{
  struct
  {
    uint8_t PRIM_RX     : 1;  // RX/TX control:  0=PTX, 1=PRX
    uint8_t PWR_UP      : 1;  // 0=Power down, 1=Power up
    uint8_t CRCO        : 1;  // CRC encoding: 0=8-bit, 1=16-bit
    uint8_t EN_CRC      : 1;  // Enable CRC
    uint8_t MASK_MAX_RT : 1;  // MAX_RT IRQ: 0=active, 1=off
    uint8_t MASK_TX_DS  : 1;  // TX_DS IRQ: 0=active, 1=off
    uint8_t MASK_RX_DR  : 1;  // RX_DR IRQ: 0=active, 1=off
    uint8_t reserved    : 1;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t ENAA_P0     : 1;  // Enable auto-ack data pipe 0
    uint8_t ENAA_P1     : 1;  // Enable auto-ack data pipe 1
    uint8_t ENAA_P2     : 1;  // Enable auto-ack data pipe 2
    uint8_t ENAA_P3     : 1;  // Enable auto-ack data pipe 3
    uint8_t ENAA_P4     : 1;  // Enable auto-ack data pipe 4
    uint8_t ENAA_P5     : 1;  // Enable auto-ack data pipe 5
    uint8_t reserved    : 2;
  } bits;
  uint8_t byte;
} NRF24L01PRegEnAA_t;
...
typedef union
{
  struct
  {
    uint8_t AW          : 2;  // RX/TX addr bytes: 0=illegal, 1=3, 2=4, 3=5
    uint8_t reserved    : 6;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t ARC    : 4;  // Auto retransmit count: 0=disabled, 1=1, ..., 15=15
    uint8_t ARD    : 4;  // Auto retransmit delay: 0=250uS, 1=500uS, ..., 15=4ms
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t RF_CH       : 7;  // Frequency channel
    uint8_t reserved    : 1;
  } bits;
  uint8_t byte;
} NRF24L01PRegRfCh_t;
...
typedef union
{
  ...
  {
    uint8_t reserved1       : 1;
    uint8_t RF_PWR          : 2;  // Output power (dBm): 0=-18, 1=-12, 2=-6, 3=0
    uint8_t RF_DR_HIGH      : 1;  // Air data rate
    uint8_t PLL_LOCK        : 1;  // Force PLL lock signal:  set to zero
    uint8_t RF_DR_LOW       : 1;  // Air data rate
    uint8_t reserved2       : 1;
    uint8_t CONT_WAVE       : 1;  // Continuous carrier transmit
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    ...
    uint8_t RX_P_NO      : 3;  // Data pipe # for RX_FIFO reading
    uint8_t MAX_RT       : 1;  // Max # TX retransmits int flag
    uint8_t TX_DS        : 1;  // Data sent int flag
    uint8_t RX_DR        : 1;  // Data received int flag
    uint8_t reserved     : 1;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t ARC_CNT      : 4;  // Count of lost packets
    uint8_t PLOS_CNT     : 4;  // Count of retransmitted packets
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t RPD          : 1;  // Receive power detector
    uint8_t reserved     : 7;  
  } bits;
  uint8_t byte;
} NRF24L01PRegRpd_t;
...
typedef union
{
  struct
  {
    uint8_t RX_PW_P0     : 6;  // Num bytes in data pipe 0
    uint8_t reserved     : 2;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t RX_PW_P1     : 6;  // Num bytes in data pipe 1
    uint8_t reserved     : 2;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t RX_PW_P2     : 6;  // Num bytes in data pipe 2
    uint8_t reserved     : 2;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t RX_PW_P3     : 6;  // Num bytes in data pipe 3
    uint8_t reserved     : 2;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t RX_PW_P4     : 6;  // Num bytes in data pipe 4
    uint8_t reserved     : 2;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t RX_PW_P5     : 6;  // Num bytes in data pipe 5
    uint8_t reserved     : 2;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    ...
    uint8_t TX_REUSE     : 1;  // Reuse last transmitted packet
    uint8_t reserved2    : 1;
  } bits;
  uint8_t byte;
...
typedef union
{
  struct
  {
    uint8_t DPL_P0       : 1;  // Enable dynamic payload data pipe 0
    uint8_t DPL_P1       : 1;  // Enable dynamic payload data pipe 1
    uint8_t DPL_P2       : 1;  // Enable dynamic payload data pipe 2
    uint8_t DPL_P3       : 1;  // Enable dynamic payload data pipe 3
    uint8_t DPL_P4       : 1;  // Enable dynamic payload data pipe 4
    uint8_t DPL_P5       : 1;  // Enable dynamic payload data pipe 5
    uint8_t reserved1    : 2;
  } bits;
  uint8_t byte;
} NRF24L01PRegDynPd_t;
...
typedef union
{
  uint8_t regArray[NRF24L01P_MAX_REGS];
  struct
  {
...
int NRF24L01PIsTxFifoFull(void);
int NRF24L01PIsRxFifoEmpty(void);
...
void NRF24L01PSetReceiveAddress(NRF24L01PPipeNum_t pipeNum, const uint8_t address[NRF24L01P_ADDR_SIZE_BYTES]);
