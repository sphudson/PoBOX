...
//
//----------------------------------------------------------------------------
uint8_t USARTReadWait(USARTDevNum_t devNum)
{
  while (!USARTRxAvailable(devNum)) { __WFI(); }; // wait for some bytes
  
  return USARTReadByte(devNum);
}
...
{
  DMA_ClearITPendingBit(DMA1_IT_GL6 | DMA1_IT_TC6);
}
...
{
  USARTDevStruct_t *devPtr = &device[USART_DEVNUM_2];
  ...
  if (devPtr->txBufferHead != devPtr->txBufferTail)
  {
    USARTSetupTxDMA(USART_DEVNUM_2);
  }
}
...
{
  USARTDevStruct_t *devPtr = &device[USART_DEVNUM_3];
  ...
  if (devPtr->txBufferHead != devPtr->txBufferTail)
  {
    USARTSetupTxDMA(USART_DEVNUM_3);
  }
}
...
void USARTInit(USARTDevNum_t devNum, uint32_t baudRate, uint8_t flowControl)
{
  ...
  switch (devNum)
  {
    ...
    case USART_DEVNUM_2:
      ...
      devPtr = &device[USART_DEVNUM_2];
      devPtr->dmaTxChannel = DMA1_Channel7;
      devPtr->dmaRxChannel = DMA1_Channel6;
      devPtr->usartDevice = USART2;
      
      BoardGPIOCfgPin(BOARD_USART2_RX_GPIO_PORT, BOARD_USART2_RX_GPIO_PIN, GPIO_Mode_IPU);
      BoardGPIOCfgPin(BOARD_USART2_TX_GPIO_PORT, BOARD_USART2_TX_GPIO_PIN, GPIO_Mode_AF_PP);
      ...
      USART_InitStructure.USART_HardwareFlowControl = 
        flowControl ? USART_HardwareFlowControl_RTS_CTS: USART_HardwareFlowControl_None;
      USART_InitStructure.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;
      USART_Init(devPtr->usartDevice, &USART_InitStructure);
      ...
      NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
      NVIC_Init(&NVIC_InitStructure);
      devPtr->dmaTxIRQChannel = DMA1_Channel7_IRQn;
      ...
      devPtr->dmaRxIRQChannel = DMA1_Channel6_IRQn;
    ...
    case USART_DEVNUM_3:
      ...
      // DMA channel 3 (USART3 RX)
      // DMA channel 2 (USART3 TX)
      devPtr = &device[USART_DEVNUM_3];
      devPtr->dmaTxChannel = DMA1_Channel2;
      devPtr->dmaRxChannel = DMA1_Channel3;
      devPtr->usartDevice = USART3;
      
      BoardGPIOCfgPin(BOARD_USART3_TX_GPIO_PORT, BOARD_USART3_TX_GPIO_PIN, GPIO_Mode_AF_PP);
      BoardGPIOCfgPin(BOARD_USART3_RX_GPIO_PORT, BOARD_USART3_RX_GPIO_PIN, GPIO_Mode_IPU);
      ...
      NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
      NVIC_Init(&NVIC_InitStructure);
      devPtr->dmaRxIRQChannel = DMA1_Channel3_IRQn;
      ...
      devPtr->dmaTxIRQChannel = DMA1_Channel2_IRQn;
